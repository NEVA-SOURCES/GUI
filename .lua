...
-- Create corner radius for UI elements
local UICorner = Create("UICorner", {CornerRadius = UDim.new(0, 6)})
local LeftPanel = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(45, 45, 48),BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 19),Size = UDim2.new(0, 131, 0, 249),ClipsDescendants = true})
Create("UICorner", {Parent = LeftPanel, CornerRadius = UDim.new(0, 6)})
local LogList = Create("ScrollingFrame",{Parent = LeftPanel,Active = true,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,BorderSizePixel = 0,Position = UDim2.new(0, 0, 0, 9),Size = UDim2.new(0, 131, 0, 232),CanvasSize = UDim2.new(0, 0, 0, 0),ScrollBarThickness = 4,ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)})
...
local RightPanel = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(30, 30, 32),BorderSizePixel = 0,Position = UDim2.new(0, 131, 0, 19),Size = UDim2.new(0, 319, 0, 249),ClipsDescendants = true})
Create("UICorner", {Parent = RightPanel, CornerRadius = UDim.new(0, 6)})
local CodeBox = Create("Frame",{Parent = RightPanel,BackgroundColor3 = Color3.fromRGB(20, 20, 22),BorderSizePixel = 0,Size = UDim2.new(0, 319, 0, 119)})
Create("UICorner", {Parent = CodeBox, CornerRadius = UDim.new(0, 4)})
local ScrollingFrame = Create("ScrollingFrame",{Parent = RightPanel,Active = true,BackgroundColor3 = Color3.new(1, 1, 1),BackgroundTransparency = 1,Position = UDim2.new(0, 0, 0.5, 0),Size = UDim2.new(1, 0, 0.5, -9),CanvasSize = UDim2.new(0, 0, 0, 0),ScrollBarThickness = 4,ScrollBarImageColor3 = Color3.fromRGB(100, 100, 100)})
...
local TopBar = Create("Frame",{Parent = Background,BackgroundColor3 = Color3.fromRGB(25, 25, 28),BorderSizePixel = 0,Size = UDim2.new(0, 450, 0, 19)})
...
local CloseButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.fromRGB(45, 45, 48),BorderSizePixel = 0,Position = UDim2.new(1, -19, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
Create("UICorner", {Parent = CloseButton, CornerRadius = UDim.new(0, 3)})
...
local MaximizeButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.fromRGB(45, 45, 48),BorderSizePixel = 0,Position = UDim2.new(1, -38, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
Create("UICorner", {Parent = MaximizeButton, CornerRadius = UDim.new(0, 3)})
...
local MinimizeButton = Create("TextButton",{Parent = TopBar,BackgroundColor3 = Color3.fromRGB(45, 45, 48),BorderSizePixel = 0,Position = UDim2.new(1, -57, 0, 0),Size = UDim2.new(0, 19, 0, 19),Font = Enum.Font.SourceSans,Text = "",TextColor3 = Color3.new(0, 0, 0),TextSize = 14})
Create("UICorner", {Parent = MinimizeButton, CornerRadius = UDim.new(0, 3)})
...
local ToolTip = Create("Frame",{Parent = SimpleSpy3,BackgroundColor3 = Color3.fromRGB(35, 35, 40),BackgroundTransparency = 0.1,BorderColor3 = Color3.fromRGB(60, 60, 65),Size = UDim2.new(0, 200, 0, 50),ZIndex = 3,Visible = false})
...
function onToggleButtonHover()
    if not toggle then
        TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = Color3.fromRGB(252, 51, 51), TextSize = 15}):Play()
    else
        TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = Color3.fromRGB(68, 206, 91), TextSize = 15}):Play()
    end
end

--- Executed when the toggle button is unhovered over
function onToggleButtonUnhover()
    TweenService:Create(Simple, TweenInfo.new(0.3), {TextColor3 = Color3.fromRGB(255, 255, 255), TextSize = 14}):Play()
end
...
function onXButtonUnhover()
    TweenService:Create(CloseButton, TweenInfo.new(0.2), {BackgroundColor3 = Color3.fromRGB(45, 45, 48)}):Play()
end
...
--- Copies all RemoteEvents in the game to clipboard
newButton(
    "Copy All Remotes",
    function() return "Click to copy all RemoteEvents and RemoteFunctions in the game to clipboard.\nThis will scan the entire game and may take a few seconds for large games." end,
    function()
        local allRemotes = {}
        ...
        local function getAllRemotes()
            ...
            for i, descendant in ipairs(descendants) do
                if descendant:IsA("RemoteEvent") or descendant:IsA("RemoteFunction") then
                    table.insert(allRemotes, v2s(descendant))
                end
                
                -- Update status every 1000 objects to show progress
                if i % 1000 == 0 then
                    TextLabel.Text = "Searching... Found " .. #allRemotes .. " remotes so far"
                    task.wait() -- Yield to prevent freezing
                end
            end
        end
        
        local success, err = pcall(getAllRemotes)
        
        if not success then
            TextLabel.Text = "Error while searching: " .. tostring(err)
            return
        end
        
        if #allRemotes > 0 then
            local remoteScript = "-- All RemoteEvents and RemoteFunctions in the game\n-- Generated by SimpleSpy\n-- Total: " .. #allRemotes .. " remotes\n\n"
            for _, remotePath in ipairs(allRemotes) do
                remoteScript = remoteScript .. remotePath .. "\n"
            end
            setclipboard(remoteScript)
            TextLabel.Text = "Copied " .. #allRemotes .. " remotes to clipboard!"
        else
